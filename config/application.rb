...
module Itask
  class Application < Rails::Application
    ...
    ITASK_CONFIG = YAML.load_file(File.join(Rails.root, 'config/itask.yml'))[Rails.env.to_s]
    config.action_mailer.delivery_method = :smtp
    config.action_mailer.perform_deliveries = true
    config.action_mailer.raise_delivery_errors = true
    config.action_mailer.default :charset => "utf-8"
    config.action_mailer.default_url_options = {host: ITASK_CONFIG['itask']['domain']}
    config.action_mailer.smtp_settings = {
        address: ITASK_CONFIG['mailer']['address'],
        port: ITASK_CONFIG['mailer']['port'],
        user_name: ITASK_CONFIG['mailer']['user_name'],
        password: ITASK_CONFIG['mailer']['password'],
